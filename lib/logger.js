...
/** Strips ANSI escape codes to get visual string length. @param {string} str @return {string} */
function viewed (str) {
  const re = /\u001b\[\d+m/gm // eslint-disable-line no-control-regex
  return String(str).replace(re, '')
}

/** Visually-aware string truncation that respects ANSI codes.
 *  Trims back to last space if far from target width.
 *  @param {string} str @param {number} amt @return {string} */
...
/** Logger factory. Returns a log function with cols and br methods.
 *  @param {object} [cons] @return {function} */
function Logger (cons) {
  ...
  /** Multi-column layout formatter. Handles numeric padding,
   *  ANSI-aware column width, and iterative row rendering. */
