...
export default class Session extends EventEmitter {
  ...
  /** Maps to vorpal.prompt for a session context. @param {object|object[]} options @param {function} cb */
  prompt (options, cb) {
    options = options || {}
  ...
  getAutocompleteDeprecated (str, cb) {
    ...
    const ap = this.parent.ui._activePrompt
    const cursor = (ap && ap.rl && ap.rl.cursor) || (ap && ap.cursor) || 0
