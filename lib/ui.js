...
class UI extends EventEmitter {
  ...
  prompt (options, cb) {
    ...
    // Bridge process.stdin through a PassThrough so @inquirer/core's createPrompt
    // doesn't directly manage process.stdin. This prevents @inquirer's readline
    // cleanup from removing listeners or closing process.stdin between sequential
    // prompts in _promptArray, which would cause the event loop to drain and exit.
    // process.stdout is passed directly so @inquirer/password can detect the TTY
    // and apply proper password masking.
    let promptInput
    let promptOutput
    if (options.context && options.context.input && options.context.output) {
      promptInput = options.context.input
      promptOutput = options.context.output
    } else {
      promptInput = new PassThrough()
      process.stdin.pipe(promptInput)
      promptOutput = process.stdout
    }

    const p = promptFn(promptOptions, { input: promptInput, output: promptOutput })
    p.then((result) => {
      if (!(options.context && options.context.input && options.context.output)) {
        process.stdin.unpipe(promptInput)
      }
      this._activePrompt = { rl: null, line: '', cursor: 0, _type: promptType }
      this._cancelled = false
      const response = {}
