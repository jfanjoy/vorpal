...
class UI extends EventEmitter {
  ...
  prompt (options, cb) {
    ...
    // Bridge process.stdin/process.stdout through PassThrough streams so
    // @inquirer/core's createPrompt doesn't directly manage process.stdin.
    // This prevents @inquirer's readline cleanup from removing listeners or
    // closing process.stdin between sequential prompts in _promptArray,
    // which would cause the event loop to drain and the process to exit.
    let promptInput
    let promptOutput
    if (options.context && options.context.input && options.context.output) {
      ...
      promptOutput.pipe(process.stdout)
    }
    ...
    p.then((result) => {
      if (!(options.context && options.context.input && options.context.output)) {
        ...
        promptOutput.unpipe(process.stdout)
      }
      this._activePrompt = undefined
      this._cancelled = false
      const response = {}
