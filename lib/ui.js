...
class UI extends EventEmitter {
  ...
  prompt (options, cb) {
    ...
    // For @inquirer prompts, pass process.stdin/process.stdout directly so
    // that readline can enter raw mode (critical for password masking etc.)
    // If the caller provides custom streams via context, use those instead.
    const promptInput = (options.context && options.context.input) || process.stdin
    const promptOutput = (options.context && options.context.output) || process.stdout
